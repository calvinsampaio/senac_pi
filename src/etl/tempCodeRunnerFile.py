...
def read_csv_if_exists(path: Path, parse_dates=None, dtype=None):
    if not path.exists():
        return None
    try:
        df = pd.read_csv(path, sep=CSV_SEP, dtype=dtype or str, low_memory=False, header=0, encoding=DEFAULT_ENCODING)
        if parse_dates:
...
def ensure_date_columns(df: pd.DataFrame, cols):
    for c in cols:
        if c in df.columns:
            df[c] = pd.to_datetime(df[c], dayfirst=True, errors='coerce', infer_datetime_format=True)
    return df
